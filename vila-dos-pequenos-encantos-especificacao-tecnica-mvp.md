...
| 4 — Coleta e crafting | Fluxo inicial implementado; validação mobile pendente | Cinco pontos fixos de madeira disponíveis desde o início e três pontos de pedra disponíveis desde o início; todos reaparecem individualmente após 3 minutos. Barra de inventário com seis espaços na base da tela, imagens e quantidade, reparo da bancada (3 madeiras e 2 pedras) bloqueado até a missão terminar, fabricação de uma cadeira (2 madeiras e 1 pedra) e save V5 com leitura de V1/V2/V3/V4. Ícone e contagem regressiva sobre cada recurso indisponível; marcadores ainda provisórios. |
...
Implementação inicial: selecionar a cadeira no inventário dentro do ateliê abre uma prévia em grade de 40 px. O jogador move a prévia por toque, setas ou botões, confirma em célula livre, aproxima-se e usa E no computador ou toca na cadeira no celular para selecioná-la com borda visível, edita a posição ou guarda a cadeira. O movimento do personagem fica pausado no editor. A posição e o estado do inventário são salvos em V5; saves V1 a V4 continuam legíveis. O recorte atual cobre uma cadeira no ateliê; outros móveis e requisitos narrativos ficam para as próximas entregas.
